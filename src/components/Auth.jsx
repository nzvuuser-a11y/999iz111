...
export default function Auth() {
  ...
  const doLogin=()=>{
    ...
    if(u.password!==password){ alert('Неверный пароль'); return; }
    saveCurrentUser(u);
  ...
  const doRegister=()=>{
    ...
    if(findUserByPhone(regPhone.trim())){
  ...
  return (
    <div className="card" style={{padding:20}}>
      ...
      {mode==='login' && <>
        ...
        <div style={{marginTop:10,cursor:'pointer'}} onClick={()=>setRecoveryOpen(!recoveryOpen)}>Забыли пароль?</div>

        {recoveryOpen && <div style={{marginTop:15}}>
          <input className="input" placeholder="Телефон" value={recoveryPhone} onChange={e=>setRecoveryPhone(e.target.value)}/>
          <button className="secondary-btn" style={{marginTop:10}} onClick={doRecover}>Восстановить</button>
          {foundPass && <div style={{marginTop:10}}>Ваш пароль: <b>{foundPass}</b></div>}
        </div>}
      </>}
      ...
      {user && <button className="secondary-btn" style={{marginTop:20}} onClick={doLogout}>Выйти из аккаунта</button>}
    </div>
  );
}
